...
def persistence_baseline(
    ...
    """
    Persistence baseline model: predicts the next value as the previous observed value.

    This implementation uses the internal time sequence of `test_df`:
    For each timestamp t, the prediction is y(t−1).

    If you later shift the target column (e.g., creating a true one-hour-ahead target),
    this function will still remain compatible as long as the temporal alignment
    is preserved.
    """
    y = test_df[target_col].values
    if len(y) < 2:
        raise ValueError("Not enough data for persistence baseline.")
    y_true = y[1:]
    y_pred = y[:-1]
